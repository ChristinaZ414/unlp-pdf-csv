...
def extract_fields(text):
    ...
    # Flight 1: Date
    flight1_date = re.search(r'Depart\s*-\s*([A-Za-z]{3} \d{1,2})', text)
    if not flight1_date:
        flight1_date = re.search(r'Depart\s*\n([A-Za-z]{3} - [A-Za-z]{3} \d{1,2})', text)
    if flight1_date:
        row[10] = flight1_date.group(1).replace(" - ", " ").strip()
    ...
    # FROM CITY (Column 13, which is 'FLIGHT 1 FROM CITY')
    ...
    # TO CITY (Column 14, which is 'FLIGHT 1 TO CITY')
    ...
    # Departure Time (FLIGHT 1 DEPARTURE TIME, column 15)
    dep_time = re.search(r'Depart\s*\n(\d{2}:\d{2})', text)
    if dep_time:
        row[15] = dep_time.group(1).replace(":", "")

    # Arrival Time (FLIGHT 1 ARRIVAL TIME, column 16)
