...
def parse_flight_block(block):
    flight = ["-"] * 7

    # Airline - line with airline name, possibly with extra text below
    airline_match = re.search(r"(Air Canada|Deutsche Lufthansa AG|Westjet|United Airlines|Porter Airlines)", block)
    flight[1] = title_case(airline_match.group(1)) if airline_match else "-"

    # Flight number - first 3-4 digit number in block
    flight_num_match = re.search(r"\b(\d{3,4})\b", block)
